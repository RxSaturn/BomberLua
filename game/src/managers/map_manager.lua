...
-- Verifica se uma posição contém uma parede destrutível
function map_manager:isDestructibleWall(gridX, gridY)
	-- Verifica se está fora dos limites
	if not grid.isWithinBounds(gridX, gridY) then
		return false
	end

	return self.tiles[gridY][gridX] == 2
end

-- Destrói uma parede na posição especificada
function map_manager:destroyWall(gridX, gridY, powerup_manager)
	...
	if self.tiles[gridY][gridX] == 2 then
		...
		-- Se o gerenciador de power-ups foi fornecido, tenta revelar um power-up
	...
	for y = 1, grid.HEIGHT do
		for x = 1, grid.WIDTH do
			...
			if tileType == 1 then -- Parede indestrutível
				visual_effects.drawSolidWall(pixelX, pixelY, grid.TILE_SIZE)
			elseif tileType == 2 then -- Parede destrutível
				visual_effects.drawBrickWall(pixelX, pixelY, grid.TILE_SIZE)
			end
		end
	end
end
