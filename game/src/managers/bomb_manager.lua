...
function bomb_manager:createExplosion(gridX, gridY, power, owner, map_manager)
        ...
        for i = 1, power do
            ...
            if map_manager:isWall(newX, newY) then
                ...
                if map_manager.tiles[newY][newX] == 1 then
                    ...
                    map_manager:destroyWall(newX, newY)
                    
                    -- Chance de gerar power-up (implementar depois)
...
function bomb_manager:update(dt, map_manager, player_manager)
    -- Atualiza as bombas
    for i = #self.bombs, 1, -1 do
        ...
        if shouldExplode then
            -- Explode a bomba
            self:createExplosion(bomb.gridX, bomb.gridY, bomb.power, bomb.owner, map_manager)
    ...
    player_manager:checkExplosionHits(self)
end
